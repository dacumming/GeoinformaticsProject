...
# a meshgrid of the slave image indexes is created, considering the boundaries of the
# master image and a padding to perform correct interpolation at the boundaries
pad=10
...
if n_model==0:
    ...
    neigh_1.fit(ij_slave, bands_slave.reshape(len(ij_slave), 5, order='F'))
    neigh_1_img=np.uint8(np.round(neigh_1.predict(ij_master).reshape(
        m_shp[0], m_shp[1], 5, order='F')))
    ...
    for i, j in np.ndindex(in_img.shape):  # loop on the indexes of the master image
        if not in_img[i, j]:
            ...
            neigh_1_img[i, j, :] = np.array([0, 0, 0, 0, 0])

elif n_model==1:
    neigh_2 = KNeighborsRegressor(n_neighbors=2)
    neigh_2.fit(ij_slave, bands_slave.reshape(len(ij_slave), 5, order='F'))
    ...
    for i, j in np.ndindex(in_img.shape):  # loop on the indexes of the master image
        if not in_img[i, j]:
            # as in the k nearest neighbor models the interpolation was computed in the paddings, now they are deleted
            neigh_2_img[i, j, :] = np.array([0, 0, 0, 0, 0])

elif n_model==2:
    neigh_3 = KNeighborsRegressor(n_neighbors=3)
    neigh_3.fit(ij_slave, bands_slave.reshape(len(ij_slave), 5, order='F'))
    ...
    for i, j in np.ndindex(in_img.shape):  # loop on the indexes of the master image
        if not in_img[i, j]:
            # as in the k nearest neighbor models the interpolation was computed in the paddings, now they are deleted
            neigh_3_img[i, j, :] = np.array([0, 0, 0, 0, 0])

elif n_model==3:
    linear_img = np.zeros((m_shp[0], m_shp[1], 5))
    # integer indexes at the nearest 16 nodes of each of the master image indexes
    i1 = np.floor(I_master).astype(int)
    i2 = i1 + 1
    j1 = np.floor(J_master).astype(int)
    j2 = j1 + 1
    ...
    for i, j in np.ndindex(in_img.shape):  # loop on the indexes of the master image
        ...
        if in_img[i, j]:
            ...
            for k in range(1,3):
                for l in range(1,3):
                    exec('p' + str(k) + str(l) + '=bands_slave[j' + str(k) + '[i, j], i' + str(k) + '[i, j]]')
    ...
    cubic_img = np.zeros((m_shp[0], m_shp[1], 5))
    ...
    for i, j in np.ndindex(in_img.shape):  # loop on the indexes of the master image
        if in_img[i, j]:
            ...
            cubic_img[i, j, :] = bicubicInterpolation(p00, p01, p02, p03, p10, p11, p12, p13,
                                                      ...
                                                      np.tile(J_master[i, j] - j1[i, j], 5),
                                                      np.tile(I_master[i, j] - i1[i, j], 5))
    # suitable format for images (8-bit integers)
    cubic_img = np.uint8(np.round(cubic_img))

elif n_model == 5:
    spline_img = np.zeros((m_shp[0], m_shp[1], 5))
...
# selected model is used to resample the slave image and save it in a raster file
img_files=['neigh_1_img', 'neigh_2_img', 'neigh_3_img', 'linear_img', 'cubic_img', 'spline_img']
with rasterio.open('dati/Avcilar_SPOTXS.img') as src:
    ras_meta = src.profile
    ras_meta['count'] = 5
with rasterio.open('dati/'+resampled_img, 'w', **ras_meta) as dst:
    dst.write(np.moveaxis(eval(img_files[n_model]), -1, 0))
